...
#### Weight tiers <Badge color="green" icon="sparkles" size="sm">Added in 1.5</Badge>
...
<Warning>
Before Lunar 1.5, weight tiers were always evaluated in kilograms regardless of the method's `weight_unit`. Methods that set a unit other than `kg` need their tier thresholds re-entered in that unit after upgrading. Methods that left `weight_unit` empty are unaffected.
</Warning>
