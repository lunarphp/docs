...
The `stephenjude/filament-two-factor-authentication` package has been removed due to an issue with Fortify and no suitable release being tagged.
You will need to replace the package with the forked version provided by Lunar.

```json
"stephenjude/filament-two-factor-authentication": "^2.0", // [!code --]
"lunarphp/filament3-2fa": "^2.1.0", // [!code ++]
```
...
Product association types have now been moved to a dedicated ENUM and the model constants have been deprecated.
The ENUM class can be switched in config to allow for extending.

#### Added meta fields to product options
The ProductOption and ProductOptionValue models now include a `meta` field. If you have added your own `meta` fields
you may experience a migration conflict.

## 1.1

### Medium Impact

#### Renamed Order resource "OrderSummaryInfolist" extension hook
There was a typo in the extension hook. You should rename `exendOrderSummaryInfolist` to `extendOrderSummaryInfolist`.
...
Look at the [model extending](/1.x/extending/models) section for all available functionality.
...
<Warning>When upgrading to `1.x` from `0.x`, please ensure you are upgraded to `0.8` first.</Warning>
...
Media definition classes allow you to register media collections, conversions and much more. See [Media Collections](/1.x/reference/media.html#media-collections)
