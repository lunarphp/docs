...
| `handle` | `string` | Unique slug identifier, auto-generated from `name` if omitted |
...
$group = CollectionGroup::create([
    'name' => 'Main Catalogue',
    'handle' => 'main-catalogue', // Auto-generated if omitted
]);
```
...
| `_lft` | `int` | Read-only nested set left pointer |
| `_rgt` | `int` | Read-only nested set right pointer |
| `parent_id` | `unsignedBigInteger` `nullable` | Parent collection for tree hierarchy |
...
| `ancestors` | HasMany | `Lunar\Models\Collection` | All parent collections up the tree (from NodeTrait) |
| `descendants` | HasMany | `Lunar\Models\Collection` | All children recursively (from NodeTrait) |
