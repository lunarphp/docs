...
|:-|:-|
|`id`||
|`title`|Mr, Mrs, Miss, etc (nullable)|
|`first_name`||
|`last_name`||
...
|Relationship|Type|Pivot Table|
|:-|:-|:-|
|`customerGroups()`|BelongsToMany|`customer_customer_group`|
|`users()`|BelongsToMany|`customer_user`|
|`discounts()`|BelongsToMany|`customer_discount`|
|`addresses()`|HasMany||
|`orders()`|HasMany||
...
Customer groups allow customers to be organised into logical segments, enabling different criteria to be defined on models based on which group a customer belongs to.

These criteria include things like:

### Pricing

Different pricing can be specified per customer group. For example, customers in the `trade` customer group may have different prices to those in `retail`.
...
// Schedule the model to be enabled straight away (starts_at defaults to null).
...
Any of these values can be overridden as they are merged behind the scenes.
