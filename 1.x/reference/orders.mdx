...
<Info>
Shipping lines do not have a purchasable. A shipping option is a data transfer object rather than a model, so from Lunar 1.5 shipping lines are stored with no morph and `$line->purchasable` returns `null`. Everything needed to display the line (`description`, `unit_price`, `total`, `meta`) is snapshotted onto the line itself.

Guard for `null` when iterating `$order->lines`, or use `$order->productLines` to exclude shipping lines:
