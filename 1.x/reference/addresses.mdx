...
Addresses belong to [Customers](/1.x/reference/customers) and are used to store shipping and billing information. A customer can have multiple addresses, with the ability to mark defaults for both shipping and billing. Addresses are linked to a [Country](#countries) and optionally a [State](#states).
...
You can also create an address directly.
...
<Tip>
When you set an address as the default, Lunar automatically unsets the previous default for that customer via an observer. You don't need to manually manage this.
</Tip>
...
<Info>
You should run this command after installing Lunar and after any updates that may include new country or state data.
</Info>
