...
$taxZone = app(GetTaxZone::class)->execute($address);
```

Passing no address (or an address with none of these fields set) returns the default tax zone. A cart can also override this resolution entirely, see [Carts → Setting the Tax Zone](/1.x/reference/carts#setting-the-tax-zone).
