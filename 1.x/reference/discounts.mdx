...
<Info>
From Lunar 1.5, `BuyXGetY` also enforces the discount's own conditions (minimum spend, customer restrictions, and `max_uses_per_user`) before awarding a reward, in the same way as `Lunar\DiscountTypes\AmountOff`. In earlier versions only `min_qty` gated the discount. See the [upgrade guide](/1.x/getting-started/overview/upgrade-guide#discount-conditions-are-now-honored-by-buyxgety).
</Info>
