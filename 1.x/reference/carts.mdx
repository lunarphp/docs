...
When `current()` is called, the behaviour depends on the `auto_create` config. With `auto_create` set to `false` (default), `null` is returned if no cart exists — preventing unnecessary database records.
