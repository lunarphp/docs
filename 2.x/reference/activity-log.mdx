...
Lunar uses the [laravel-activitylog](https://spatie.be/docs/laravel-activitylog) package by Spatie under the hood, wrapped in a custom trait at `Lunar\Core\Models\Concerns\LogsActivity`. This trait configures the following defaults:
...
AttributeGroup, Brand, Cart, CartAddress, CartLine, Channel, Collection, CollectionGroup, Currency, Customer, CustomerGroup, Discount, Fulfilment, FulfilmentLine, FulfilmentTracking, Location, Order, OrderAddress, OrderLine, Product, ProductOption, ProductType, ProductVariant, RefundLine, Staff, Tag, TaxClass, TaxRate, TaxZone, and Transaction.
