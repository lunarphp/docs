...
#### Overriding the tax zone <Badge color="green" icon="sparkles" size="sm">Added in 1.5</Badge>
...
$price->priceExTax($zone);
$price->comparePriceIncTax($zone);
```

This is useful when displaying product prices for a known visitor region before the cart is built. Once items are in the cart, the cart's own `tax_zone_id` (see [Setting the Tax Zone](/1.x/reference/carts#setting-the-tax-zone)) drives tax-inclusive line totals.
