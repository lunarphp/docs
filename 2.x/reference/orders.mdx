...
Lunar ships default emails for the order lifecycle (confirmation on placement, payment received, shipped, ready for collection, provisioned, return received, cancelled, and refund issued), each overridable or removable from a service provider. See the [Notifications](/2.x/reference/notifications) reference for the full set, the notification manifest, mail routing, and how to customize the templates.
