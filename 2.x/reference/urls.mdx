...
| `slug` | `string` | The URL slug. Indexed |
| `default` | `boolean` | Whether this is the default URL for this element and language combination. Indexed |
...
| `language` | BelongsTo | `Lunar\Core\Models\Language` | The language this URL is for |
...
Lunar ships with URL support on the following models, via the `Lunar\Core\Models\Concerns\HasUrls` trait:
...
<Info>
When a model using the `HasUrls` trait is deleted, all associated URLs are automatically removed.
</Info>

## Retrieving a model by slug

Since URLs are polymorphic and slug lookups are typically scoped to a language, resolve a model from a slug by querying `Url` and loading its `element`:
...
$product = Url::where('slug', 'apple-iphone')
...
The built-in `Lunar\Core\Generators\UrlGenerator` performs the following steps when a model is created:

1. Checks whether the model already has any URLs (skips generation if it does).
2. Reads the model's `name` (via `translate('name')`, so translatable `name` columns resolve to the current locale).
