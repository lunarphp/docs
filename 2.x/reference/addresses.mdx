...
Addresses belong to [Customers](/2.x/reference/customers) and are used to store shipping and billing information. A customer can have multiple addresses, with the ability to mark defaults for both shipping and billing. Addresses are linked to a [Country](/2.x/reference/countries-states#countries) and have an optional `state` string field for storing state or province information.

## Addresses

```php
Lunar\Core\Models\Address
```
...
```php
$customer = \Lunar\Core\Models\Customer::find(1);
...
```php
$address = \Lunar\Core\Models\Address::create([
...
|:-|:-|:-|:-|
|`customer`|`BelongsTo`|`Lunar\Core\Models\Customer`|The customer who owns this address|
|`country`|`BelongsTo`|`Lunar\Core\Models\Country`|The country this address is in|
