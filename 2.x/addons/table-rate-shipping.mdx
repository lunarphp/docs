...
The add-on's config is merged under the `lunar.shipping-tables` key from its own `config/shipping-tables.php`:
...
| `shipping_rate_tax_calculation` | `'default'` | Tax class strategy for shipping rates. Use `'default'` for the system default tax class, `'highest'` to use the highest tax rate found among cart items, or a callable (e.g., `[App\Shipping\TaxRateCalculator::class, 'calculate']`) for custom logic. Callables receive the cart and should return a `Lunar\Core\Models\TaxClass`. |

This config is merged, not published — there is no `vendor:publish` tag for it. To override a value, create `config/lunar/shipping-tables.php` in the host application with the keys to change.
...
The permission is created automatically by a migration when the add-on is installed. See [Access Control](/2.x/admin/access-control) for guidance on assigning permissions to roles and staff.
...
| countries | BelongsToMany | `Lunar\Core\Models\Country` | Countries included in this zone |
| states | BelongsToMany | `Lunar\Core\Models\State` | States included in this zone |
| postcodes | HasMany | `ShippingZonePostcode` | Postcodes included in this zone |
| orders | BelongsToMany | `Lunar\Core\Models\Order` | Orders assigned to this zone |
...
| data | `json` `nullable` | Driver-specific configuration and the availability schedule (cast to `AsArrayObject`; stored as `jsonb` on PostgreSQL) |
...
| customerGroups | BelongsToMany | `Lunar\Core\Models\CustomerGroup` | Customer groups that can see and use this method |
...
A shipping rate connects a shipping method to a shipping zone and holds the pricing. It implements the `Lunar\Core\Contracts\Purchasable` interface and uses the `HasPrices` trait, meaning prices are stored in Lunar's core `prices` table with support for tiered pricing and multiple currencies.
...
| prices | MorphMany | `Lunar\Core\Models\Price` | Tiered pricing via the `HasPrices` trait |
...
Shipping methods use an availability schedule to define when the method can be selected, managed through the Shipping Method page in the admin panel. The schedule is stored in the method's `data` column under a `schedule` key, keyed by ISO weekday number (`1` for Monday through `7` for Sunday). Each day entry has:

| Key | Description |
|:----|:------------|
| `enabled` | Whether the method is available on this day |
| `from` | Time the method becomes available, as `H:i` (or `null` for no lower bound) |
| `to` | Time the method stops being available, as `H:i` (or `null` for no upper bound) |

A method with no schedule configured is always available. When the cart is evaluated, the shipping rate resolver checks the schedule for each method and only returns rates from methods that are currently available.
...
Each shipping method uses a driver to determine how pricing is resolved. The add-on ships with four built-in drivers, all implementing `Lunar\Shipping\Interfaces\ShippingRateInterface`.
...
|:----|:-------|:------------|
| `charge_by` | `cart_total` (default), `weight` | Determines what value is used to match against price tiers. When set to `weight`, each cart line's weight is converted into the method's `weight_unit` and multiplied by its quantity. |

#### Weight tiers

When `charge_by` is set to `weight`, tiers are evaluated in the unit configured as the shipping method's `weight_unit`, defaulting to `kg` when it is empty. Each cart line's weight is converted from its own purchasable unit first, so a cart mixing items measured in grams and kilograms is totaled correctly.

Tier thresholds are stored as whole numbers. To express a threshold below one kilogram, configure the method in a smaller unit: a method with a `weight_unit` of `g` can use tiers of `500` and `1000` where kilograms would have required `0.5` and `1`. The admin panel shows the configured unit alongside the tier field and rejects decimal values rather than truncating them.
...
Every driver also checks the cart's products against any exclusion list attached to the shipping zone, returning `null` (no option) if a product in the cart is excluded.
...
A shipping discount holds one or more rules, stored under the discount's `data.methods` key. Each rule may target a specific shipping method (by `shipping_method_id`) or act as a catch-all (no method selected), and offers one of two adjustments:
...
The rate resolver finds applicable shipping rates for a cart, filtering by customer group, availability schedule, stock availability, and weight constraints.
...
|:---------|:-----|:------------|
| `cart` | `Lunar\Core\Models\Cart` | The cart being resolved |
| `shippingRate` | `ShippingRate` | The shipping rate that was resolved |
| `shippingOption` | `Lunar\Core\DataTypes\ShippingOption` | The resulting shipping option |

## Order Observer

When an order is created or updated, the add-on automatically resolves the shipping zone from the order's shipping address and attaches it to the order via the `order_shipping_zone` pivot table. The zone name is also stored in the order's meta and added as a `shipping_zone` searchable attribute for [search indexing](/2.x/reference/search).
