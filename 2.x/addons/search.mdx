...
`getParams()` returns the merged overrides. `DatabaseEngine` has no request parameters and ignores them.

### Meta fields

Both data objects carry a free-form `meta` array for engines and stages to annotate:

- `SearchHit::$meta` holds the engine's own score under `score` where the engine provides one: Typesense `text_match`, Meilisearch `_rankingScore` (the engine requests it with `showRankingScore`). The database engine leaves it unset. Stages add their own keys.
