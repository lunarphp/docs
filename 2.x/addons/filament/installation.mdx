...
<Note>
Running the core installer, `php artisan lunar:install`, performs these steps automatically — it publishes assets and offers to create the first admin account.
</Note>
...
This prompts for a name, email, and password, and creates a staff member with full admin rights. The command also accepts `--firstname`, `--lastname`, `--email`, and `--password` options for non-interactive use.
