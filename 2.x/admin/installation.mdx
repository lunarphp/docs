...
If `lunarphp/admin` (the Filament panel) is also installed, its `lunar:create-admin` Artisan command creates a staff account with `admin` set to `true` and prompts for a name, email, and password. Otherwise, create one directly:
