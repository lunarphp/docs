...
`src/ExampleSection.php` in `lunarphp/panel-addon-example` registers three slots: a plain banner on the Customers edit page, a loyalty card on the same page that takes part in the customer's save through a [draft slice](/2.x/admin/extending/draft-slices), and the canonical SEO card on the product edit page:
...
public function slots(SlotRegistry $registry): void
{
    ...
    // to the LoyaltyTierSlice registered in draftExtensions() through
    // useDraftSlice('example-addon'), so its field autosaves and commits
...
The example card keeps its state local to stay a pure slot demonstration. A real SEO add-on would join the product's edit draft with a [draft slice](/2.x/admin/extending/draft-slices), as the loyalty card does on the customer page, so its fields save with the product.
...
- [Draft Slices](/2.x/admin/extending/draft-slices) — letting a slot component's fields autosave and commit with the page's record.
