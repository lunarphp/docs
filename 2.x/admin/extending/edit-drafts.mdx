...
Registering a `DraftableResource` is for a record the add-on owns outright. To add fields to a first-party record's existing draft, such as a loyalty tier on the customer edit page, register a [draft slice](/2.x/admin/extending/draft-slices) instead. A slice joins the record's draft under a namespace of its own and gets the same autosave, conflict detection, and atomic commit, without being able to touch the record's own fields.
...
`initial` is merged with the page's shared `draftSliceValues` prop, so any [draft slices](/2.x/admin/extending/draft-slices) registered on the record seed the form automatically. Pass `slices: false` on a page that drafts some record other than the route's deepest binding.
