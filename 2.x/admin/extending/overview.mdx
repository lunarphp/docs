...
| `formSlices(): array` | `FormSlice` classes the panel's own sections register under a bare namespace on a model. |
| `formExtensions(): array` | `FormSlice` classes an add-on contributes to a first-party form's save, placed under the reserved `addon:{key}` namespace. |
...
`Section::draftables()`, `formSlices()`, `formExtensions()`, `discountTypeForms()`, `searchSources()`, `searchCommands()`, and `widgets()` are also available on `SectionExtension`.
