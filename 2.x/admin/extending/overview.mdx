...
| `draftSlices(): array` | `DraftSlice` classes the panel's own sections register under a bare namespace on a draftable model. |
| `draftExtensions(): array` | `DraftSlice` classes an add-on contributes to a first-party edit draft, placed under the reserved `addon:{key}` namespace. |
...
`Section::draftables()`, `draftSlices()`, `draftExtensions()`, `discountTypeForms()`, `searchSources()`, `searchCommands()`, and `widgets()` are also available on `SectionExtension`.
