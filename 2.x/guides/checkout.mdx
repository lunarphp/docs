...
Once a shipping address is set, the available shipping methods can be retrieved using the `ShippingManifest` facade. Shipping options are configured by extending Lunar's shipping system (see [Extending Shipping](/2.x/extending/shipping)).
...
| `price` | `Lunar\Core\DataObjects\PriceValue` | The cost of this shipping method |
...
See the [Cart guide](/2.x/guides/cart) for the full list of computed properties available on the cart and its lines.
...
For a full walkthrough of driver selection, the offline driver, and the first-party Stripe and PayPal drivers, see the [Payment Integration guide](/2.x/guides/payment-integration).
...
Order money fields (`sub_total`, `discount_total`, `shipping_total`, `tax_total`, `total`) are stored as plain integers in the currency's minor unit. Format them with the model's `format()` method rather than treating them as objects.
...
@if($order->shippingLines->isNotEmpty())
    ...
    @foreach($order->shippingLines as $line)
        <p>{{ $line->description }}: {{ $line->format('total') }}</p>
    @endforeach
@endif
...
- Review the [Payment Integration guide](/2.x/guides/payment-integration) for driver selection, the Stripe and PayPal drivers, and post-payment order state.
- Review the [Extending Shipping](/2.x/extending/shipping) guide for adding custom shipping options.
- Review the [Extending Payments](/2.x/extending/payments) guide for building a custom payment driver.
- Review the [Extending Orders](/2.x/extending/orders) guide for customizing the order creation pipeline.
- Review the [Cart guide](/2.x/guides/cart) for building the cart page that precedes checkout.
