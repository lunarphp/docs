...
<Info>
Cart prices are produced by the calculation pipeline and, after each run, persisted to the cart and line rows so later requests can be served without running it again. `current()` accepts a `calculate` argument (`true` by default) that calculates the cart before returning it. To force the pipeline to run after making changes, call `$cart->recalculate()`. See [Persisted totals](/2.x/reference/carts#persisted-totals) for when the stored figures are served.
</Info>
