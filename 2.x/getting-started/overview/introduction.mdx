...
- **A new admin panel, built on Inertia + Vue.** Alongside the existing Filament-based admin, a first-party `lunarphp/panel` package ships a Vue 3 + Inertia.js v3 admin panel with its own design system and a runtime extension mechanism for add-on packages. The two panels coexist during the transition.
