...
<Tip>
For a step-by-step walkthrough of setting up the monorepo inside a Laravel application, see the [Local Development](/2.x/getting-started/overview/local-development) guide.
</Tip>
...
Lunar uses [Pest](https://pestphp.com/) for its test suite. Each package has its own testsuite, and CI runs them one at a time. Run a testsuite from the repository root:

```bash
vendor/bin/pest --testsuite core --parallel
```

See [Local Development](/2.x/getting-started/overview/local-development) for the full list of testsuites. All existing tests must continue to pass, and new features or bug fixes should include corresponding test coverage.
...
<Warning>
Security vulnerabilities should not be reported as public issues. See the [Security](/2.x/getting-started/overview/security) page for responsible disclosure instructions.
</Warning>
