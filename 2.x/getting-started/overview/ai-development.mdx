...
<Warning>
A Laravel Boost skill for Lunar v2 has not been published yet. The rest of this page (llms.txt, the MCP server, and Markdown export) applies to v2 already.
</Warning>
...
<Info>The MCP server supports filtering by documentation version using a `version` parameter (e.g., `v2.x`, `v1.x`, or `v0.x`).</Info>
...
```
https://docs.lunarphp.com/2.x/reference/carts.md
```
