...
- **Cart totals columns** — adds the persisted totals snapshot columns (`sub_total`, `total`, `revision`, `calculated_at`, and the rest) to `carts` and `cart_lines`. No backfill: every upgraded cart reads as not yet calculated and is filled in by its next `calculate()`.
...
- **`Cart::calculate()` may serve persisted totals.** v2 persists cart totals after each calculation and serves them from the row for up to `lunar.cart.totals.ttl` seconds (default 300) when nothing on the cart has changed since. Code that needs a guaranteed-fresh total, such as a payment amount check, should call `recalculate()`. Code that writes cart, line, or address rows through the query builder must call `$cart->invalidateTotals()` afterwards. See [Persisted totals](/2.x/reference/carts#persisted-totals). Relatedly, `Cart::updateLine()` now saves the line through the model, so `CartLine` observers and activity logging fire for quantity updates as they already did for `add()`.
