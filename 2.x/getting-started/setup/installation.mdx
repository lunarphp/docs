...
| Package | Description |
|:--------|:-------------|
| `lunarphp/panel` | The Inertia.js admin panel. |
| `lunarphp/admin` | A turnkey Filament v5 admin panel. |
| `lunarphp/filament` | Filament components, widgets, and schemas for building a custom Filament v5 panel. |
...
- Creates a staff admin user (via `lunar:create-admin`, if a Filament-based panel such as `lunarphp/admin` or `lunarphp/filament` is installed)
...
- Publishes Filament assets (if Filament is installed)

<Warning>
The admin-user and Filament-asset steps assume a Filament-based panel is installed. If installing only `lunarphp/panel` (Inertia), follow [Admin Panel Installation](/2.x/admin/installation) for creating the first staff account.
</Warning>
