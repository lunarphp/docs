...
The normaliser decides which raw queries count as the same query. The default lowercases, strips punctuation, collapses whitespace, and folds simple plurals, and leaves part numbers unstemmed. A store whose customers use their own vocabulary can fold synonyms into a canonical form so that both spellings learn from each other:
...
class TopThreeOnlyRanker implements Ranker
{
    ...
    {
        $scores = $this->signals->scores($context, $hits->productIds());
