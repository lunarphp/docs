...
The cart branch calls `recalculate()`, not `calculate()`. `calculate()` may serve a persisted totals snapshot up to `lunar.cart.totals.ttl` seconds old, so a price or discount change since the payment intent was created could go unnoticed. The guard exists to catch exactly that, so it must compare against a total the pipeline has just produced. See [Persisted totals](/2.x/reference/carts#persisted-totals).
